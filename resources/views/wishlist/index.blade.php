...
<div class="container my-5" style="max-width: 1100px; margin: auto; padding: 20px; font-family: sans-serif;">
    ...
    @if($elementi->isEmpty())
        <div style="text-align: center; padding: 60px 20px; background: #fdfdfd; border: 1px dashed #ccc; border-radius: 10px;">
            ...
            <a href="{{ route('home') }}" style="display: inline-block; margin-top: 15px; background: #3498db; color: white; padding: 10px 25px; border-radius: 5px; text-decoration: none;">Sfoglia i Libri</a>
        </div>
    @else
                ...
                @foreach($elementi as $item)
                    <tr style="border-bottom: 1px solid #eee;">
                        <td style="padding: 15px;">
                            <span style="font-weight: bold; color: #34495e;">{{ $item->titolo }}</span>
                        </td>
                        <td style="padding: 15px; text-align: right; font-weight: bold; color: #2c3e50;">
                            {{ number_format($item->prezzo, 2, ',', '.') }}€
                        </td>
                        <td style="padding: 15px; text-align: center; display: flex; justify-content: center; gap: 15px; align-items: center;">
                            
                            <form action="{{ route('carrello.add', $item->id_libro) }}" method="POST" style="margin: 0;">
                                @csrf
                                <button type="submit" style="background: #27ae60; color: white; border: none; padding: 6px 15px; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 0.9rem;">
                                    🛒 Aggiungi al Carrello
                                </button>
                            </form>

                            <form action="{{ route('wishlist.remove', $item->id_libro) }}" method="POST" style="margin: 0;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" style="background: none; border: none; color: #e74c3c; cursor: pointer; font-size: 1.2rem;" title="Rimuovi">
                                    🗑️
                                </button>
                            </form>
